...
const Recipes = () => {
  const { recipes, loading, setLoadedMeals, like, unlike, isLiked } =
    useGlobalContext();
  ...
  return (
    <div className="recipes">
      ...
      {loading ? (
        ...
        <div
          ref={recipesRef}
          className="grid sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 mt-5"
        >
                  ...
                  <div>
                    {isLiked[`${index}`] ? (
                      <AiFillHeart
                        style={{ color: '#DE1A1A' }}
                        onClick={() =>
                          unlike(index, idMeal, strMeal, strMealThumb)
                        }
                      />
                    ) : (
                      <AiOutlineHeart
                        onClick={() =>
                          like(index, idMeal, strMeal, strMealThumb)
                        }
                      />
                    )}
                  </div>
